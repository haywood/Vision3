...
bool intensity_at(int img1, int img2)
{
    return getPixel(_images+img1, _i, _j) > getPixel(_images+img2, _i, _j);
}

bool zero_at(int img)
{
    return 0 == getPixel(_images+img, _i, _j);
}

int map_normal(float x)
{
    return 255.0f*(1.0f + x);
}
...
int main(int argc, char *argv[])
{
    ...
    vector<int>::iterator first_zero, it;
    float x, y, z, magnitude, albedo_max = 0, *albedo_store=(float *)calloc(sizeof(float), rows*cols),
    ...
    for (_i = 0; _i < rows; ++_i) {
        for (_j = 0; _j < cols; ++_j) {
            if (getPixel(&mask, _i, _j)) {
                ...
                first_zero = find_if(image_indices.begin(), image_indices.end(), zero_at);
                if (first_zero - image_indices.begin() >= DIM) {
                    for (int i = 0; i < DIM; ++i) {
                        k = image_indices[i];
                    ...
                    setPixelColor(&normals, _i, _j, map_normal(x), map_normal(y), map_normal(z));
                    albedo_store[_i*cols + _j] = magnitude;
                    gradients[_i*cols + _j].p = M[0]/M[2];
                    gradients[_i*cols + _j].q = M[1]/M[2];
